...
export default function EventsShow({ event }: { event: Event }) {
    ...
    // Initialize form with existing availabilities for the current user
    const initialAvailabilities = event.date_options.map(option => {
        const existing = option.availabilities.find(a => a.user_id === user.id);
        return {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        // Filter out empty statuses before submitting
        const validAvailabilities = data.availabilities.filter(a => a.status !== '');
    ...
    const updateAvailability = (dateOptionId: number, status: string) => {
        const newAvailabilities = [...data.availabilities];
        const index = newAvailabilities.findIndex(a => a.date_option_id === dateOptionId);

        if (index !== -1) {
            newAvailabilities[index].status = status;
        } else {
            newAvailabilities.push({ date_option_id: dateOptionId, status });
        }

        setData('availabilities', newAvailabilities);
    };

    // Get unique users who have voted
    const usersWhoVoted = Array.from(
        new Set(
            event.date_options.flatMap(opt =>
                opt.availabilities.map(a => a.user.id)
            )
        )
    ).map(id => {
        const availability = event.date_options.flatMap(opt => opt.availabilities).find(a => a.user.id === id);
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody className="divide-y">
                                    {/* Other users' votes */}
                                    {usersWhoVoted.map(u => {
                                        if (u?.id === user.id) return null; // Skip current user in this section
                                        return (
                                            ...
                                            return (
                                                <td key={option.id} className="px-6 py-4 text-center">
                                                    <ToggleGroup
                                                        ...
                                                        value={currentVote}
                                                        onValueChange={(value) => {
                                                            if (value) updateAvailability(option.id, value);
                                                        }}
